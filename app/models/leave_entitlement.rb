class LeaveEntitlement < ActiveRecord::Base
  attr_accessible :balance, :day, :id, :leave_type_id, :taken, :year
  
  self.table_name = 'leave_entitlement'
  
  belongs_to :leave_type
  
  validates_presence_of :balance, :message => 'Balance is required'
  ...
  validates_numericality_of :balance, :greater_than_or_equal_to => 0, :message => 'Balance is invalid'
